...
DEFINE_LOG_CATEGORY(LogCustomEOS);
...
void FUserManagerEOS::LoginWithDeviceID(const FOnlineAccountCredentials& AccountCredentials)
{
	...
	LoginInfo.DisplayName = "MyDisplayName";
	...
	CallbackObj->CallbackLambda = [LocalUserNum, UserCredentials, this, AccountCredentials](const EOS_Connect_LoginCallbackInfo* Data)
	{
		if (Data->ResultCode == EOS_EResult::EOS_Success)
		{
			UE_LOG(LogCustomEOS, Error, TEXT("Login With Device ID Success!"));
			AddLocalUser(LocalUserNum, nullptr, Data->LocalUserId, AccountCredentials);
			FUniqueNetIdEOSPtr NetIdEOS = GetLocalUniqueNetIdEOS(LocalUserNum);
			TriggerOnLoginCompleteDelegates(LocalUserNum, true, *NetIdEOS, FString());
		}
		...
		{
			UE_LOG(LogCustomEOS, Error, TEXT("Login With Device ID Failed due to %hs"), EOS_EResult_ToString(Data->ResultCode));
		}
	};
...
void FUserManagerEOS::CreateDeviceID(const FOnlineAccountCredentials& AccountCredentials)
{
	...
	DeviceIdOptions.DeviceModel = "GetDeviceModel";
	...
	CallbackObj->CallbackLambda = [LocalUserNum, AccountCredentials,this](const EOS_Connect_CreateDeviceIdCallbackInfo* Data)
	{
		...
		{
			UE_LOG(LogCustomEOS, Display, TEXT("Device ID Created"));
			LoginWithDeviceID(AccountCredentials);
		}
		else
		{
			UE_LOG(LogCustomEOS, Error, TEXT("Create Device ID Failed due to %hs"), EOS_EResult_ToString(Data->ResultCode));
		}
	};
...
void FUserManagerEOS::DeleteDeviceID()
{
	...
	EOS_Connect_OnDeleteDeviceIdCallback DeleteDeviceDelegate = [](const EOS_Connect_DeleteDeviceIdCallbackInfo* Data)
	{
		if (Data->ResultCode == EOS_EResult::EOS_Success)
		{
			UE_LOG(LogCustomEOS, Display, TEXT("Delete Device ID Success"));
		}
		else
		{
			UE_LOG(LogCustomEOS, Error, TEXT("Delete Device ID Failed due to %hs"), EOS_EResult_ToString(Data->ResultCode));
		}
	};

	EOS_Connect_DeleteDeviceId(EOSSubsystem->ConnectHandle, &DeleteDeviceIdOptions, nullptr, DeleteDeviceDelegate);
}
